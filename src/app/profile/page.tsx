...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      let u: PPUser | null = null;
      try {
        const raw = localStorage.getItem(PP_USER_KEY);
        if (raw) u = JSON.parse(raw) as PPUser;
      } catch {
        u = null;
      }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#050505] px-4">
        <p className="text-center text-[#B0B0B0]">
          We could not load your profile. Run the database migration or try again later.
        </p>
            ...
            <div className="min-w-0 flex-1">
              {editing ? (
                ...
                <>
                  ...
                  <p className="text-sm text-[#B0B0B0]">{profile.email ?? '—'}</p>
